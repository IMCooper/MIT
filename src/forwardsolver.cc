...
// the parallel setup    
     omp_threads = config->getOMPNumThreads();
     omp_chunkSize = config->getOMPChunkSize();
     omp_collapseLoops = config->getOMPNumColapseLoops();
...
//       }
      ...
      int chunk = omp_chunkSize;
      const int tmp_collapseLoops = omp_collapseLoops;
...
//     block indices:
       ...
       for (unsigned int q_point=0; q_point<n_q_points; ++q_point)
       {
        #pragma omp parallel for schedule(dynamic, chunk) collapse(1) private (block_index_i, block_index_j) reduction(+:     num_opps)
