...
namespace Step20
{
                                    ...
                                    Vector<double>   &values) const
  {
    ...
    {
      values(2) = 0.0;
      values(3) = -(alpha*p[0]*p[1]*p[1]/2 + beta*p[0] - alpha*p[0]*p[0]*p[0]/6);
    }
  }
  ...
  {
    // List of meshes to test on:
    // Cube: WORKS
//     GridGenerator::hyper_cube (triangulation, -1, 1);
//     triangulation.refine_global (2); // WORKS
//     GridTools::distort_random (0.2, triangulation, false); //WORKS
    
    // Rectangle: WORKS
//     GridGenerator::hyper_rectangle (triangulation,
//                                     Point<dim> (-1.0, -1.0, -1.0),
//                                     Point<dim> (2.0, 1.0, 1.0));
//     triangulation.refine_global (1); //WORKS    
//     GridTools::distort_random (0.2, triangulation, false); //WORKS
...
//     GridTools::distort_random (0.2, triangulation, false); //WORKS
    ...
    // BREAKS HERE:
//     Cylinder
    GridGenerator::cylinder (triangulation, 0.5, 1.0);
//     GridReordering<dim>::reorder_cells(triangulation);
//     static const CylinderBoundary<dim> cyl_boundary (0.5);// ALSO BREAKS WITHOUT CORRECT BOUNDARY:
//     triangulation.set_boundary (0, cyl_boundary);
//     triangulation.refine_global(1); // ALSO BREAKS WITHOUT REFINEMENT
  ...
  {
    QGauss<dim>   quadrature_formula(2*degree+3);
    QGauss<dim-1> face_quadrature_formula(2*degree+3);
    ...
    for (; cell!=endc; ++cell)
      {
        ...
        for (unsigned int face_no=0;
             ...
             ++face_no)
        {
          std::cout << face_no << ", " << cell->face_orientation(face_no) << std::endl;
  ...
  void MixedLaplaceProblem<dim>::compute_errors () const
  {
    ...
    QTrapez<1>     q_trapez;
    QIterated<dim> quadrature (q_trapez, 2*degree+4);
  ...
  try
    {
      ...
      MixedLaplaceProblem<3> mixed_laplace_problem(p_order);
