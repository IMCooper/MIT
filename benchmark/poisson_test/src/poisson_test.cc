                                  ...
                                  const unsigned int /*component*/) const
{
  ...
  {
    return_value += -12.0 * std::pow(p(i), 2.0);
  }
...
template <int dim>
Step4<dim>::Step4 (unsigned int order_in)
  :
  triangulation (Triangulation<3>::MeshSmoothing::none, false),
  fe (order_in),
...
{
  InputTools::read_in_mesh("../mesh/cube_broken.ucd",
//                            "../mesh/cylinder.ucd",
//                            "../mesh/cube_in_cube.ucd",
                           triangulation);
...
{
  QGauss<dim>  quadrature_formula(2*p_order+3);
